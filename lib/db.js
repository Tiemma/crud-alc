...
module.exports = function () {
	...
	var db = _mongoose2.default.createConnection('mongodb://localhost:27017/crud');
	...
	var department = db.model('Department', (0, _department2.default)());
	...
	var Departments = function () {
		...
		_createClass(Departments, null, [{
			...
			value: function find(id, projections) {
				...
				var dataCallBack = new Promise(function (resolve, reject) {
					if (id) department.find({ "id": id }, projections, function (err, person) {
						if (err) {
							reject(err);
						} else {
							resolve(person);
						}
					});else department.find({ available: true }, projections, function (err, persons) {
						if (err) {
							reject(err);
						} else {
							resolve(persons);
